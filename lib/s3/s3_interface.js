...
var methods =  {
  ...
  addParamsToUrl: function(url, params) {
    ...
    return result + querystring.stringify(params || {})
  },
  ...
  requestBuckets: function(options) {
    ...
    options.callback = function(result) {
      ...
      if (options.bucketName) {
        var meta = {}
        result.listbucketresult.keys.each(function(key) { if(key != "name") meta[key] = result.listbucketresult[key] })
        ...
        callbackResult = result.listallmybucketsresult.buckets.bucket.map(function(bucketEntry) {
          var meta = {}
          bucketEntry.keys.each(function(key) { if(key != "name") meta[key] = result.listbucketresult[key] })
...
methods.each(function(method, methodName) {
  exports.S3Interface.prototype[methodName] = method
})